package io.github.arimac.idextractionlibrary.utils;
...
public class OpenCVConnector {
    ...
    public static Observable<io.github.arimac.idextractionlibrary.values.ImageMat> resize(io.github.arimac.idextractionlibrary.values.ImageMat imageMat, float requestWidth, float requestHeight) {
    ...
    public static Observable<io.github.arimac.idextractionlibrary.values.ImageMat> getRgbMat(io.github.arimac.idextractionlibrary.values.ImageMat imageMat, byte[] data, Camera camera) {
        return Observable.create(sub -> {
            try {
    ...
    public static Observable<io.github.arimac.idextractionlibrary.values.ImageMat> getMonochromeMat(io.github.arimac.idextractionlibrary.values.ImageMat imageMat) {
        return Observable.create(sub -> {
            long now = System.currentTimeMillis();
    ...
    public static Observable<io.github.arimac.idextractionlibrary.values.ImageMat> getContoursMat(io.github.arimac.idextractionlibrary.values.ImageMat imageMat) {
        return Observable.create(sub -> {
            long now = System.currentTimeMillis();
            ...
            for (int i = 0; i < contours.size(); i++) {
                ...
                if (points.size() == 4 && mincos >= -0.3 && maxcos <= 0.5) {
                    for (int j = 0; j < points.size(); j++) {
                        Core.circle(imageMat.resizeMat, points.get(j), 6, new Scalar(255, 0, 0), 6);
                    }
    ...
    public static Observable<io.github.arimac.idextractionlibrary.values.ImageMat> getPath(io.github.arimac.idextractionlibrary.values.ImageMat imageMat) {
